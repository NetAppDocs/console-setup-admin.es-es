...
.Podman
--
Instalar y configurar Podman:

* Habilitar e iniciar el servicio podman.socket
* Instalar Python3
* Instalar el paquete podman-compose correspondiente
* Agregue podman-compose a la variable de entorno PATH
* Verifique que su versión de Podman esté usando Netavark Aardvark DNS en lugar de CNI
...
. Instalar Podman.
+
Obtenga Podman de los repositorios de Red Hat Enterprise Linux.
...
----
. Instale EPEL para obtener podman-compose.
+
.. Si utiliza Red Hat Enterprise 9.1 a 9.6:
+
[source, cli]
----
...
+
Instale una versión compatible de Docker Engine siguiendo los pasos. No instale la última versión, ya que la consola no la admite.
...
Debe proporcionar permisos de AWS a la NetApp Console mediante una de las siguientes opciones:
...
NOTE: Si quieres configurar un proxy transparente, puedes hacerlo después de la instalación. link:reference-agent-maint-console.html["Obtenga información sobre la consola de mantenimiento del agente."]
