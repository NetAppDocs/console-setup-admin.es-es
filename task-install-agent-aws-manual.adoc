...
---
= Despliega manualmente un agente de Console en AWS para NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Puedes implementar manualmente un agente de la Console en un host Linux que se esté ejecutando en AWS. Para implementar manualmente el agente de la Console en tu propio host Linux, necesitas revisar los requisitos del host, configurar tu red, preparar los permisos de AWS, implementar el agente de la Console y luego proporcionar los permisos que preparaste.
...
.Antes de empezar
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["Aprende cuándo implementar manualmente en AWS"].
...
Límite de salto de respuesta PUT al usar IMDSv2:: Si IMDSv2 está habilitado (el valor predeterminado para las nuevas instancias de EC2), establezca el límite de saltos de respuesta PUT en 3.  Si no lo hace, el sistema muestra un error de inicialización de la interfaz de usuario durante la configuración del agente.
+
--
...
+
En función de los servicios de datos de NetApp que planeas usar, puede que necesites crear una segunda política. Para las regiones estándar, los permisos se reparten en dos políticas. Se necesitan dos políticas debido a un límite de tamaño máximo de caracteres para las políticas administradas en AWS. link:reference-permissions-aws.html["Obtenga más información sobre las políticas de IAM para el agente de consola"].
...
Si tienes buckets de Amazon S3 en la misma cuenta de AWS donde creaste el agente de la Console, verás que aparece automáticamente un sistema de almacenamiento de Amazon S3 en la página *Systems*. https://docs.netapp.com/us-en/storage-management-s3-storage/index.html["Aprenda a administrar los buckets S3 desde la NetApp Console"^]
