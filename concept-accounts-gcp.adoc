...
---
= Infórmate sobre los proyectos y los permisos de Google Cloud en NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Debe implementar un agente de consola utilizando una cuenta de Google que tenga permisos para iniciar el agente de consola desde la consola.
. Al desplegar el agente de la Consola, se te pide que selecciones un https://cloud.google.com/iam/docs/service-accounts["cuenta de servicio"^] para el agente. La Consola obtiene permisos de la cuenta de servicio para crear y gestionar sistemas Cloud Volumes ONTAP, para gestionar copias de seguridad usando NetApp Backup and Recovery, y más. Los permisos se proporcionan adjuntando un rol personalizado a la cuenta de servicio.
