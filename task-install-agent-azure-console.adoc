...
---
= Implementa un agente de consola en Azure desde NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Para desplegar un agente de la Consola en Azure desde la NetApp Console, necesitas configurar tu red, preparar los permisos de Azure y luego desplegar el agente de la Consola.

Al desplegar un agente en Azure, se inicia una VM que ejecuta Linux y el software del agente de Console en una VNet de tu elección.


NOTE: Despliega un agente de Console en la misma región de Azure que los sistemas Cloud Volumes ONTAP que administra, o en la https://docs.microsoft.com/en-us/azure/availability-zones/cross-region-replication-azure#azure-cross-region-replication-pairings-for-all-geographies["Par de regiones de Azure"^]. Esto asegura que se use una conexión Azure Private Link entre Cloud Volumes ONTAP y sus cuentas de almacenamiento asociadas.
...
. En la página *Overview*, selecciona *Deploy agent > Azure*.
...
. En la página *Autenticación de máquina virtual*, selecciona la opción de autenticación que coincida con la forma en que configuraste los permisos de Azure:
+
...
+
+ El método de autenticación de la máquina virtual puede ser una contraseña o una clave pública SSH.

+
+ https://learn.microsoft.com/en-us/azure/virtual-machines/linux-vm-connect?tabs=Linux["Obtenga información sobre cómo conectarse a una máquina virtual Linux en Azure"^]

. En la página *Detalles*, introduce un nombre para el agente, especifica las etiquetas y elige si quieres que la Consola cree un nuevo rol que tenga los permisos necesarios, o si prefieres seleccionar un rol existente que configuraste con link:reference-permissions-azure.html["los permisos requeridos"].
+
...
Si tiene Azure Blob Storage en la misma cuenta de Azure donde creó el agente de consola, verá que Azure Blob Storage aparece automáticamente en la página *Sistemas*. https://docs.netapp.com/us-en/bluexp-blob-storage/index.html["Aprenda a administrar Azure Blob Storage desde la NetApp Console"^]
