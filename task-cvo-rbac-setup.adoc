...
Tras actualizar un agente de la consola a la versión 4.9.0 o posterior, la consola asigna automáticamente roles de Cloud Volumes ONTAP a los usuarios existentes en función de sus roles en la plataforma. Esta asignación única, llamada seeding, ayuda a los usuarios a conservar el acceso adecuado a Cloud Volumes ONTAP sin que sea necesario que un administrador asigne cada rol manualmente. Seeding asigna el rol de administrador o de visualizador de Cloud Volumes ONTAP en función del rol de plataforma existente del usuario. Las asignaciones solo se aplican durante la migración; un rol de plataforma no sustituye a un rol explícito de Cloud Volumes ONTAP en las nuevas asignaciones. Revisa los roles asignados inicialmente para confirmar que los usuarios conservan únicamente el acceso que necesitan.

.Pasos
. Identifica la organización, la carpeta o el proyecto que contiene cada sistema Cloud Volumes ONTAP.
. Revisa la función de Cloud Volumes ONTAP asignada a cada usuario y comprueba que esté asignada a nivel de organización, carpeta o proyecto que contenga el sistema. Para las funciones predefinidas previstas, consulta link:reference-iam-cvo-roles.html#seeding-mappings-for-existing-users["Configuración inicial de las asignaciones para los usuarios existentes"].
