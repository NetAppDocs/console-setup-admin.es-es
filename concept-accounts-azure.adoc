...
Al implementar un agente de consola desde la consola, debe usar una cuenta de Azure o una entidad de servicio que tenga permisos para implementar la máquina virtual del agente de consola.  Los permisos necesarios se enumeran en ellink:task-install-connector-azure-bluexp.html#connector-custom-role["Política de implementación de agentes para Azure"] .
