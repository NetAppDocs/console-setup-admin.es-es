...
---
= Permisos de Google Cloud para el agente de la NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
| config.deployments.get | No | Sí | Sí 
...
| crear.discos.computar .8+| Para crear y gestionar discos en sistemas Cloud Volumes ONTAP. | Sí | Sí | No 
...
| compute.forwardingRules.update | Actualiza las reglas de reenvío existentes para la gestión del tráfico. | No | Sí | No 
...
| registro.logEntries.create .2+| Crea y enruta entradas de registro para supervisión, depuración y auditoría. | Sí | Sí | No 
...
| cloudbuild.workerpools.get | Para acceder a la información del grupo de trabajadores durante el despliegue en modo privado y la conversión de sistemas Cloud Volumes ONTAP usando Infrastructure Manager. | Sí | Sí | Sí 


| cloudbuild.workerpools.list | Para listar la información del grupo de trabajadores durante el despliegue en modo privado de Cloud Volumes ONTAP usando Infrastructure Manager. | Sí | No | No 
...
Se añade el permiso `compute.disks.update` para actualizar los parámetros de IOPS y rendimiento en un agregado o disco Hyperdisk Balanced en sistemas Cloud Volumes ONTAP.
...
Los permisos `cloudbuild.workerpools.get` y `cloudbuild.workerpools.list` se añaden para dar soporte a Infrastructure Manager en implementaciones en modo privado de Cloud Volumes ONTAP en Google Cloud.
...
El permiso `compute.forwardingRules.update` se añade para dar soporte a Infrastructure Manager en implementaciones de Cloud Volumes ONTAP en Google Cloud.
