...
---
= Despliega el agente de NetApp Console en modo restringido
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
link:task-prepare-restricted-mode.html#step-3-review-host-requirements["Requisitos del agente de revisión"].

* Un par de claves para la instancia EC2.


.Pasos
. Ve a https://aws.amazon.com/marketplace/pp/prodview-jbay5iyfmu6ui["Listado de agentes de la NetApp Console en AWS Marketplace"^].
...
+
Si utiliza una dirección IP de SKU estándar, la consola utiliza la dirección IP _privada_ del agente de la consola, en lugar de la IP pública. Si la máquina que utiliza para acceder a la consola no puede alcanzar la dirección IP privada, la consola no funciona.
...
--
Proporciona a la consola NetApp las credenciales de la entidad de servicio principal de Azure que configuraste anteriormente.
...
.Resultado
La NetApp Console ahora tiene los permisos que necesita para realizar acciones en Azure en tu nombre.
