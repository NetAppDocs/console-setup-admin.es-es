...
[role="lead"]
Agrega y administra credenciales de AWS para que puedas implementar y gestionar recursos en la nube en tus cuentas de AWS desde la NetApp Console. Si gestionas varias suscripciones de AWS Marketplace, puedes asignar cada una de ellas a diferentes credenciales de AWS desde la página Credentials.
...
link:concept-accounts-aws.html["Descubra cómo la NetApp Console utiliza las credenciales y los permisos de AWS"].
...
NOTE: Si implementó un agente de consola desde la consola, se agregaron automáticamente las credenciales de AWS para la cuenta en la que implementó un agente de consola. Esto garantiza que se cuente con los permisos necesarios para administrar los recursos.
...
Si quieres proporcionar a la consola claves de AWS para un usuario de IAM, entonces necesitas conceder los permisos necesarios a ese usuario. La política de IAM de la consola define las acciones y recursos de AWS que la consola puede usar.
...
NOTE: Las nuevas credenciales de su proveedor de nube pueden tardar unos minutos en estar disponibles.
...
Configura una función de IAM que permita que la aplicación de software como servicio (SaaS) NetApp Console asuma la función.
...
----
+
Consulta link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies-cross-account-resource-access.html["Documentación de AWS Identity and Access Management (IAM)"^] para más información sobre el acceso a recursos entre cuentas en IAM.
...
.Resultado
El rol IAM ahora tiene los permisos necesarios. <<add-the-credentials,Ahora puedes agregarlo a la consola>>.
...
.Antes de empezar
Si acabas de crear el rol IAM, pueden pasar unos minutos hasta que esté disponible para su uso. Espera unos minutos antes de agregar las credenciales a la NetApp Console.
...
. En la página *Credenciales de la organización*, seleccione *Agregar credenciales* y siga los pasos del asistente.
+
.. *Ubicación de credenciales*: seleccione *Amazon Web Services > Consola*.
...
Para más información, consulta el https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Documentación de la consola para Amazon FSx para ONTAP"^]
...
.Antes de empezar
Necesitas crear un agente de Console antes de poder configurar una suscripción. link:concept-agent-install-options.html["Aprenda a crear un agente de consola"].
...
Cuando te suscribes desde AWS Marketplace, el último paso del proceso es asociar la suscripción con tu organización. Si no completaste este paso, entonces no puedes usar la suscripción con tu organización.
...
.Pasos
. Confirme que no asoció su suscripción con su organización de la consola.
+
...
. Confirme que la suscripción esté asociada a su organización.
+
...
.Pasos
. Seleccione *Administración > Credenciales*.
. En la página *Credenciales de la organización*, seleccione el menú de acciones para un conjunto de credenciales y luego seleccione *Editar credenciales*.
...
TIP: No se pueden eliminar las credenciales de un perfil de instancia que esté asociado con un agente de consola.
