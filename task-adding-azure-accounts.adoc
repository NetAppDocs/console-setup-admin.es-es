...
image:diagram_azure_authentication.png["Imagen conceptual que muestra la consola obteniendo autenticación y autorización de Microsoft Entra ID antes de poder realizar una llamada API. La función Consola define permisos en Active Directory. Está vinculado a una o más suscripciones de Azure y a un objeto de entidad de servicio que representa la aplicación de consola."]
...
.Resultado
Puedes cambiar a otro conjunto de credenciales desde la página Detalles y credenciales https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-deploying-otc-azure.html["al agregar un sistema a la consola"^].
