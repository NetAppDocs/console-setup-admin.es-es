...
summary: 'Obtenga información sobre las configuraciones predeterminadas del agente de consola para implementaciones estándar (con acceso a Internet) en AWS, Azure y Google Cloud, así como para implementaciones restringidas (sin acceso a Internet) en entornos locales.' 
---
= Configuración predeterminada del agente de la consola en NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* El servicio de agente de consola se denomina `occm`.
* El servicio `occm` depende del servicio MySQL.
+
Si el servicio MySQL está caído, entonces el `occm` servicio también lo está.
