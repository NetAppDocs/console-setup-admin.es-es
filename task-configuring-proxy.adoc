...
---
= Configura un agente de NetApp Console para usar un servidor proxy
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
IMPORTANT: Una vez que hayas configurado un proxy, no puedes cambiar el tipo de proxy. Si necesitas cambiar el tipo de proxy, debes eliminar el agente de la NetApp Console y agregar un nuevo agente con el nuevo tipo de proxy.
...
Si configuraste un agente de la consola para usar un servidor proxy, puedes habilitar el tráfico directo de API en el agente de la consola para enviar llamadas de API directamente a los servicios del proveedor de la nube sin pasar por el proxy. Los agentes que se ejecutan en AWS, Azure o Google Cloud admiten esta opción.
