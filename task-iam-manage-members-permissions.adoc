...
Puede agregar dos tipos de miembros a su organización: una cuenta de usuario y una cuenta de servicio. Las aplicaciones utilizan cuentas de servicio para realizar tareas de API automáticamente.  Las personas usan cuentas de usuario para iniciar sesión y administrar recursos.
...
Cuando se elimina un miembro, el sistema revoca sus permisos pero conserva sus cuentas de consola y del sitio de soporte de NetApp .
...
. Seleccione *Recrear*.
. Descargue o copie el ID del cliente y el secreto del cliente. + La consola muestra el secreto del cliente solo una vez. Cópielo o descárguelo y guárdelo de forma segura.
...
Los usuarios deben configurar MFA nuevamente cuando inicien sesión después de la eliminación. Si el usuario solo perdió el acceso a su dispositivo MFA temporalmente, puede usar el código de recuperación que guardó cuando configuró MFA para iniciar sesión.
